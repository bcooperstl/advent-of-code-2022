...
{
#ifdef DEBUG_DAY_20
    printf("Working node %d %d **%d** %d %d\n", ring_node->prev->prev->value, ring_node->prev->value, ring_node->value, ring_node->next->value, ring_node->next->next->value);
#endif
    ...
    if (ring_node->value > 0)
    {
        ...
        // remove ring_node from current spot in ring
        ring_node->next->prev = ring_node->prev;
        ring_node->prev->next = ring_node->next;
...
#ifdef DEBUG_DAY_20
    printf("After moving node %d %d **%d** %d %d\n", ring_node->prev->prev->value, ring_node->prev->value, ring_node->value, ring_node->next->value, ring_node->next->next->value);
#endif
}
...
#ifdef DEBUG_DAY_20
        printf("Node %d has previous node %d and next node %d\n", i, (index->num_nodes+i-1)%index->num_nodes, (i+1)%index->num_nodes);
#endif
