...
static int check_overlap(int first_low, int first_high, int second_low, int second_high)
{
    // check if first lower than second and has overlap
    if ((first_low <= second_low) && (first_high >= second_low))
    {
#ifdef DEBUG_DAY_4
        printf("The first region of %d-%d overlaps the second region of %d-%d\n", first_low, first_high, second_low, second_high);
#endif
        return TRUE;
    }
    // check if second lower than first and has overlap
    if ((second_low <= first_low) && (second_high >= first_low))
    {
#ifdef DEBUG_DAY_4
        printf("The second region of %d-%d overlaps the first region of %d-%d\n", second_low, second_high, first_low, first_high);
#endif
        return TRUE;
    }
#ifdef DEBUG_DAY_4
    printf("The first region of %d-%d and the second region of %d-%d do not overlap\n", first_low, first_high, second_low, second_high);
#endif
    return FALSE;
}
...
void day_4_part_2(char * filename, extra_args_t * extra_args, char * result)
{
    int count = 0;
    file_data_t fd;
    ...
    while (current != NULL)
    {
        ...
        if (check_overlap(first_low, first_high, second_low, second_high) == TRUE)
        {
            count++;
        }
